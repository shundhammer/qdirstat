...
QDirStatApp::initActions()
{
    ...
    _fileAskOpenUrl	= new KAction( tr( "Open &URL..." ), "konqueror", 0,
    ...
    _fileRefreshAll		= new KAction( tr( "Refresh &All" ), "reload", 0,
					       this, SLOT( refreshAll() ),
					       actionCollection(), "file_refresh_all" );

    _fileRefreshSelected	= new KAction( tr( "Refresh &Selected" ), 0,
					       this, SLOT( refreshSelected() ),
					       actionCollection(), "file_refresh_selected" );

    _fileReadExcludedDir	= new KAction( tr( "Read &Excluded Directory" ), 0,
					       this, SLOT( refreshSelected() ),
					       actionCollection(), "file_read_excluded_dir" );

    _fileContinueReadingAtMountPoint = new KAction( tr( "Continue Reading at &Mount Point" ), "hdd_mount", 0,
						    this, SLOT( refreshSelected() ), actionCollection(),
						    "file_continue_reading_at_mount_point" );

    _fileStopReading	= new KAction( tr( "Stop Rea&ding" ), "stop", 0,
				       this, SLOT( stopReading() ), actionCollection(),
				       "file_stop_reading" );

    _fileAskWriteCache	= new KAction( tr( "&Write to Cache File..." ), "fileexport", 0,
				       this, SLOT( askWriteCache() ), actionCollection(),
				       "file_ask_write_cache" );

    _fileAskReadCache	= new KAction( tr( "&Read Cache File..." ), "fileimport", 0,
    ...
    _cleanupOpenWith	= new KAction( tr( "Open With" ), 0,
				       this, SLOT( cleanupOpenWith() ),
				       actionCollection(), "cleanup_open_with" );

    _treemapZoomIn 	= new KAction( tr( "Zoom in" ), "viewmag+", Key_Plus,
				       this, SLOT( treemapZoomIn() ),
				       actionCollection(), "treemap_zoom_in" );

    _treemapZoomOut 	= new KAction( tr( "Zoom out" ), "viewmag-", Key_Minus,
				       this, SLOT( treemapZoomOut() ),
				       actionCollection(), "treemap_zoom_out" );

    _treemapSelectParent= new KAction( tr( "Select Parent" ), "up", Key_Asterisk,
				       this, SLOT( treemapSelectParent() ),
				       actionCollection(), "treemap_select_parent" );

    _treemapRebuild 	= new KAction( tr( "Rebuild Treemap" ), 0,
				       this, SLOT( treemapRebuild() ),
				       actionCollection(), "treemap_rebuild" );

    _showTreemapView	= new KToggleAction( tr( "Show Treemap" ), Key_F9,
					     this, SLOT( toggleTreemapView() ),
					     actionCollection(), "options_show_treemap" );

    new KAction( tr( "Help about Treemaps" ), "help", 0,
    ...
    _reportMailToOwner	= new KAction( tr( "Send &Mail to Owner" ), "mail_generic", 0,
				       _treeView, SLOT( sendMailToOwner() ),
				       actionCollection(), "report_mail_to_owner" );

    _helpSendFeedbackMail = new KAction( tr( "Send &Feedback Mail..." ), 0,
					 this, SLOT( sendFeedbackMail() ),
					 actionCollection(), "help_send_feedback_mail" );


    _fileAskOpenDir->setStatusText	( tr( "Opens a directory"	 		) );
    _fileAskOpenUrl->setStatusText	( tr( "Opens a (possibly remote) directory"	) );
    _fileOpenRecent->setStatusText	( tr( "Opens a recently used directory"	) );
    _fileCloseDir->setStatusText	( tr( "Closes the current directory" 		) );
    _fileRefreshAll->setStatusText	( tr( "Re-reads the entire directory tree"	) );
    _fileRefreshSelected->setStatusText	( tr( "Re-reads the selected subtree"		) );
    _fileReadExcludedDir->setStatusText ( tr( "Scan directory tree that was previously excluded" ) );
    _fileContinueReadingAtMountPoint->setStatusText( tr( "Scan mounted file systems"	) );
    _fileStopReading->setStatusText	( tr( "Stops directory reading"		) );
    _fileAskWriteCache->setStatusText	( tr( "Writes the current directory tree to a cache file that can be loaded much faster" ) );
    _fileAskReadCache->setStatusText	( tr( "Reads a directory tree from a cache file" ) );
    _fileQuit->setStatusText		( tr( "Quits the application" 		) );
    _editCopy->setStatusText		( tr( "Copies the URL of the selected item to the clipboard" ) );
    _cleanupOpenWith->setStatusText	( tr( "Open file or directory with arbitrary application" ) );
    _showTreemapView->setStatusText	( tr( "Enables/disables the treemap view" 	) );
    _treemapZoomIn->setStatusText	( tr( "Zoom treemap in"		 	) );
    _treemapZoomOut->setStatusText	( tr( "Zoom treemap out"		 	) );
    _treemapSelectParent->setStatusText	( tr( "Select parent"			 	) );
    _treemapRebuild->setStatusText	( tr( "Rebuild treemap to fit into available space" ) );
    pref->setStatusText			( tr( "Opens the preferences dialog"		) );
    _reportMailToOwner->setStatusText	( tr( "Sends a mail to the owner of the selected subtree" ) );
}
...
{
    statusBar()->insertItem( tr( "Ready." ), ID_STATUS_MSG );
}
...
{
    statusMsg( tr( "Opening directory..." ) );
...
QDirStatApp::fileAskOpenDir()
{
    ...
    KURL url = KFileDialog::getExistingDirectory( QString::null, this, tr( "Open Directory..." ) );

    if( ! url.isEmpty() )
	openURL( QDir::cleanPath( url.url() ) );
...
{
    statusMsg( tr( "Closing directory..." ) );
...
{
    statusMsg( tr( "Refreshing directory tree..." ) );
...
QDirStatApp::refreshSelected()
{
    ...
    statusMsg( tr( "Refreshing selected subtree..." ) );
...
QDirStatApp::askWriteCache()
{
	...
	file_name =
	    KFileDialog::getSaveFileName( DEFAULT_CACHE_NAME, 			// startDir
					  QString::null,			// filter
					  this,					// parent
					  tr( "Write to Cache File" ) );	// caption
	...
	if ( access( file_name, F_OK ) == 0 )	// file exists
	{
		...
		KMessageBox::questionYesNoCancel( this,
						  tr( "File %1 exists. Overwrite?" ).arg( file_name ),
						  tr( "Overwrite?" ) );	// caption
    ...
    statusMsg( tr( "Writing cache file..." ) );

    if ( ! _treeView || ! _treeView->writeCache( file_name ) )
    {
	QString errMsg = tr( "Error writing cache file %1" ).arg( file_name );
	statusMsg( errMsg );
	KMessageBox::sorry( this, errMsg,
			    tr( "Write Error" ) );			// caption
    }

    statusMsg( tr( "Wrote cache file %1" ).arg( file_name ) );
}
...
QDirStatApp::askReadCache()
{
	...
	KFileDialog::getOpenFileName( DEFAULT_CACHE_NAME,		// startDir
				      QString::null,			// filter
				      this,				// parent
				      tr( "Read Cache File" ) );	// caption

    statusMsg( tr( "Reading cache file..." ) );
...
QDirStatApp::askForFeedback()
{
    ...
    switch ( KMessageBox::warningYesNoCancel( this,
					      tr( "Now that you know this program for some time,\n"
						    "wouldn't you like to tell the authors your opinion about it?\n"
						    "\n"
						    "Open Source software depends on user feedback.\n"
						    "Your opinion can help us make the software better." ),
					      tr( "Please tell us your opinion!" ),	// caption
					      tr( "Open &Feedback Form..." ),		// yesButton
					      tr( "&No, and don't ask again!" )	// noButton
					      )
	     )
    {
