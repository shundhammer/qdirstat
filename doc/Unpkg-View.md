...
This shows some files that obviously belong to Qt 5 on openSUSE Tumbleweed, but
that are not in the file list of any package. It is still possible that some
...
believe that the unpackaged files are all that is there, and maybe remove that
entire directory and wreck the system.


![Unpackaged Files Screenshot](https://github.com/shundhammer/qdirstat/blob/master/screenshots/QDirStat-unpkg-boot-grub2.png)

This shows a custom Grub2 font that was manually created to have a larger, much
better readable boot font.  That one file clearly stands out from all the
packaged files there.

`/boot/grub2` and `/etc` are some of those directories that contain a wild
mixture of package and unpackaged files; some are generated, some are just
backups of the previous version.


### Ignored Files

In this view, all files that are in the file list of any installed package are
installed. As a result, there are many directories that don't contain any
...
way. Sometimes version numbers may change in incompatible ways.

If you are using a Linux distribution for a long time, you will get many
...
Before you nuke your complete installation and reinstall, consider having a
...
without using your package manager. It might just be part of the crucial driver
for that fancy wireless keyboard or mouse that you use, and using your system
without that hardware may be no fun.
...
- Does a package upgrade / uninstall behave properly? Are all files that the
  old version installed or created during its usage properly removed? Or is
...
when you click on it; the details panel on the right show the package name.
...
belong to a package. Of course that makes sense only for system directories.

If there are unpackaged files in system directories, this may be a bug, or it
may be normal. But it is a starting point for further investigation and for
asking people who may know more about that.

Linux (and BSD and other Unix-like systems) were meant to be open and easy to
explore. Become involved. Become knowledgeable what all that stuff on your
system is. You may be able to contribute finding problems; you don't need to be
a developer for that. Just exploring things and asking questions may help the
community to identify problems.
