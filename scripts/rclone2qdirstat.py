...
if __name__ == "__main__":
    ...
    with open(input_file) as csv_fp, open(output_file, 'w') as out_fp:
        out_fp.write(f'''[qdirstat 1.0 cache file]
...
''')
        ...
        for row in csv_reader:
            try:
                ...
                sys.stderr.write(f'Error reading {input_file}:{csv_reader.line_num}: {ex}.\nDid you run "rclone lsf" with "--format=stmp" and --csv parameters?\n')
                sys.exit(1)
            out_fp.write(f'{item_type}\t{formatted_path}\t{formatted_size}\t{mtime_unix_seconds}\n')
