...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            f"""Usage: {sys.argv[0]} <input CSV file> <output file>
...
"""
        ...
        for row in csv_reader:
            try:
                ...
                item_type = (
                    "D" if row["mimetype"].startswith("inode/directory") else "F"
                )
                assert row["path"], "path must not be empty"
                formatted_path = "/" + urllib.parse.quote(row["path"])
                mtime_unix_seconds = int(
                    datetime.strptime(row["mtime"], "%Y-%m-%d %H:%M:%S").timestamp()
                )
            except Exception as ex:
                sys.stderr.write(
                    f"""Error reading {input_file}:{csv_reader.line_num}: {ex}.
Did you run "rclone lsf" with "--format=stmp" and --csv parameters?
"""
                )
                sys.exit(1)
            out_fp.write(
                f"{item_type}\t{formatted_path}\t{formatted_size}\t{mtime_unix_seconds}\n"
            )
